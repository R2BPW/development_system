...
% Длинное имя проекта с разрешённым переносом по подчёркиванию
\newcommand{\devs}{\texttt{development\allowbreak\_\allowbreak system}}
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\textit{\devs}}
\fancyhead[R]{\small\textit{Порождение вычислительных потоков}}
\fancyfoot[C]{\thepage}
...
\begin{center}
  ...
  \begin{abstract}
    \normalsize
    В настоящей работе описана двухуровневая система \devs,
...
требующему от пользователя глубокого знания API. \devs
...
% ============================================================
\section{Конфигурация оркестратора: \texttt{dusha.rkt}}

\subsection{Формат конфигурационного файла}

Поведение Мастера задаётся конфигурационным файлом \texttt{душа.scm} в формате
S-выражения. Файл содержит четыре поля:
...
\midrule
\texttt{указание} & строка & System prompt для языковой модели \\
\texttt{модель}   & строка & Идентификатор модели на OpenRouter \\
\texttt{теплота}  & число  & Temperature (\texttt{0.0}--\texttt{2.0}) \\
\texttt{память}   & список & Предпочтения пользователя (user preferences) \\
\texttt{девиз}    & строка & Стилевые директивы генерации кода \\
\bottomrule
\end{tabular}
\caption{Поля конфигурации Мастера}
\end{table}

\begin{lstlisting}[language=Racket, caption={Конфигурационный файл \texttt{душа.scm}}]
(конфиг
  (system-prompt  "Ты помощник для создания вычислительных потоков
                   на Common Lisp.")
  (модель         "claude-sonnet-4-20250514")
  (temperature    0.3)
  (preferences    ((стиль . "рекурсия, однострочники, SICP")))
  (style-guide    "краткость, без англицизмов, функциональный стиль"))
\end{lstlisting}

Выбор S-выражений как формата мотивирован единообразием: весь проект оперирует
данными в этом формате — метафайлы потоков, файл активного потока, трасс-логи.
Альтернативы (TOML, JSON, YAML) потребовали бы отдельных парсеров.

\subsection{Операции над конфигурацией: \texttt{dusha.rkt}}

Модуль \texttt{dusha.rkt} реализует типичный функциональный CRUD для
неизменяемых S-выражений. Доступ к полю — через паттерн-матчинг:

\begin{lstlisting}[language=Racket, caption={Чтение поля из S-выражения (dusha.rkt)}]
(define (получить-поле конфиг имя)
  (match конфиг
    [(list _ поля ...)          ; первый элемент — тег (конфиг/душа/etc.)
...
Обновление реализовано как структурное преобразование без мутации:

\begin{lstlisting}[language=Racket, caption={Функциональное обновление поля конфига}]
(define (заменить-поле конфиг имя значение)
  (match конфиг
    [(list тег поля ...)
     (cons тег
...
Тег (\texttt{конфиг}, \texttt{мета}, \texttt{след}) хранится как первый элемент
и не участвует в обходе полей — это даёт одну реализацию \texttt{получить-поле}
для всех S-выражений проекта.
...
Мастер формирует задание для Claude Code, объединяя поля конфигурации
(\texttt{system-prompt}, \texttt{style-guide}) с пользовательским описанием
и явными техническими требованиями:
...
Доступ к полям метафайла через паттерн-матчинг аналогичен \texttt{получить-поле} из \texttt{dusha.rkt}:
...
(define (запустить-опрос)
  (загрузить-конфиг)      ; читает душа.scm → *конфиг*
...
В работе описана система \devs, реализующая концепцию
...
\begin{lstlisting}[basicstyle=\ttfamily\footnotesize, numbers=none,
                   caption={Полная структура каталогов \devs}]
development_system/
├── мастер/
