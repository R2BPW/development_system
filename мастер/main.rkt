...
;; --- состояние UI (SICP-style: явный тип, чистые переходы) ---

(struct бот-состояние (ожидают) #:transparent)
;;   ожидают : immutable hash  ид-чата → #t

(define начальное-состояние (бот-состояние (hash)))

(define (добавить-ожидающего с ид)
  (бот-состояние (hash-set (бот-состояние-ожидают с) ид #t)))

(define (убрать-ожидающего с ид)
  (бот-состояние (hash-remove (бот-состояние-ожидают с) ид)))

(define (ожидает? с ид)
  (hash-ref (бот-состояние-ожидают с) ид #f))
...
;; Обработка одного сообщения. Чистая относительно состояния:
;; принимает с, возвращает новое-с. Побочные эффекты — только отправка.
(define (обработать-событие текст ид-чата с)
  ...
  (cond
    ...
    [(string=? текст "🔧 Породить поток")
     ...
     (добавить-ожидающего с ид-чата)]
    [(ожидает? с ид-чата)
     (породить-и-отправить! ид-чата текст)
     (убрать-ожидающего с ид-чата)]
    [(string=? текст "/отчёт")
     (with-handlers ([exn:fail?
                      (lambda (e)
                        (послать-сообщение ид-чата (format "Ошибка: ~a" (exn-message e))))])
       (let ((пдф (собрать-отчёт)))
         (if пдф
             (послать-документ ид-чата пдф "Журнал вычислений")
             (послать-сообщение ид-чата "Нет данных трассировки."))))
     с]
    [else
...
(define (запустить-опрос)
  ...
  (let цикл ((сдвиг (загрузить-сдвиг)) (с начальное-состояние))
    (with-handlers ([exn:break?
      ...
      (let* ((ответ   (получить-обновления сдвиг))
        ...
        (let обр ((ост события) (макс сдвиг) (с с))
          (if (null? ост)
              ...
              (let* ((событие (car ост))
                     ...
                     (тип     (third событие))
                     (новое-с (if (and (админ? ид-чата) (eq? тип 'message))
                                  (обработать-событие (fourth событие) ид-чата с)
                                  с)))
                (обр (cdr ост) (max макс (+ ид-обн 1)) новое-с))))))))
