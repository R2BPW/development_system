...
(define (обработать-диалог текст)
  (let ((д (текущая-душа)))
    (if (not д)
        (string-append "Не могу ответить — душа не загружена. " текст)
        (with-handlers ([exn:fail?
                         (lambda (e)
                           (format "Ошибка при обращении к модели: ~a" (exn-message e)))])
          (let* ((указание (or (получить-поле д 'указание) "Ты помощник."))
                 ...
                 (сообщения (list (hash 'role "user" 'content текст))))
            (спросить-модель полное-указание модель теплота сообщения))))))
    ...
    (if (not поток)
        ...
        (let* ((мета (third поток))
               (описание (if мета
                             ...
                             "без описания"))
               (результат-запуска (обработать-запустить имя)))
          (if (string-contains? результат-запуска "не найден")
              результат-запуска
...
(define (разобрать-пары-ветвления части)
  (with-handlers ([exn:fail? (lambda (_) #f)])
    (for/list ([ч (in-list части)])
      (let ((раздел (string-split ч ":" #:trim? #f)))
        (if (>= (length раздел) 2)
            (list (first раздел) (string-join (rest раздел) ":"))
