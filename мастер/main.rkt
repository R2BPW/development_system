...
(define (сформировать-ответ текст)
  (cond
    ...
    [(string-prefix? текст "/породить ") 'породить-inline]
...
(define (породить-и-отправить! ид-чата описание)
  "Порождает поток, шлёт статус + файл .lisp в чат."
  (let ((д (текущая-душа)))
    (if (not д)
        (послать-сообщение ид-чата "Конфиг не загружен.")
        (with-handlers ([exn:fail?
                         (lambda (e)
          ...
          (let* ((р      (породить-поток д описание (каталог-потоков)))
                 ...
                 (путь   (third р))
                 (значок (if успех? "✅" "⚠️"))
                 (сообщ  (format "Поток «~a» порождён ~a." имя значок)))
            (displayln (format "> ~a" сообщ))
            (послать-с-кнопками ид-чата сообщ)
            ;; Гарантированно шлём .lisp файл
            (when (file-exists? путь)
              (послать-документ ид-чата путь
                (format "~a — ~a" имя описание))))))))

(define (обработать-породить описание)
  "Устаревший вариант — возвращает строку (для обратной совместимости)."
  (let ((д (текущая-душа)))
    (if (not д) "Конфиг не загружен."
        (with-handlers ([exn:fail? (lambda (e) (format "Ошибка: ~a" (exn-message e)))])
          (let* ((р (породить-поток д описание (каталог-потоков))))
            (format "Поток «~a» порождён." (second р)))))))
                  ...
                  (let ((текст (fourth событие)))
                    ...
                    (cond
                      ...
                      [(hash-ref *ожидает-породить* ид-чата #f)
                       ...
                       (породить-и-отправить! ид-чата текст)]
                      [(string=? текст "/отчёт")
                       (with-handlers ([exn:fail?
                                        (lambda (e)
                                          (послать-сообщение ид-чата
                      ...
                      [else
                       (let ((р (сформировать-ответ текст)))
                         (cond
                           ;; /породить — side-effect здесь, где есть ид-чата
                           [(eq? р 'породить-inline)
                            (породить-и-отправить! ид-чата (substring текст 10))]
                           [р
                            (displayln (format "> ~a" р))
                            (записать-историю (каталог-истории) ид-чата текст р)
